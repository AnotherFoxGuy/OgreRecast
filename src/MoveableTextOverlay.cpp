...
void MovableTextOverlay::_computeTextWidth()
{
	...
	for(Ogre::String::iterator i = mCaptionWidthCalc.begin(); i < mCaptionWidthCalc.end();i++)
	{   
		if (*i == 0x0020)
...
void MovableTextOverlay::update(Real timeSincelastFrame)
{
	...
	Real relTextHeight = (mAttrs->getCharacterHeight() * mCaptionLines) / Ogre::OverlayManager::getSingleton().getViewportHeight();
