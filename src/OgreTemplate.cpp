	...
	recalcActiveTile(true), mCurrentSkybox(SKYBOX_NONE), m_drawPortals(true), m_tileSet(0)
{
	...
	master_time = new Time();
	master_database = new Database();
	master_msgroute = new MsgRoute();
	master_debuglog = new DebugLog();
}
//-------------------------------------------------------------------------------------
OgreTemplate::~OgreTemplate(void)
{	
 ...
 else
 {
	...
	if(geom)
	{
		...
		if(m_sampleToolType != TOOL_NONE)
		{
			if(m_tool)
			{
				m_tool->handleRender(evt.timeSinceLastFrame);
			}
		}
	}
...
bool OgreTemplate::keyPressed( const OIS::KeyEvent &arg )
{
	...
	if(m_sampleToolType == TOOL_NAVMESH_TESTER)
	{
		if(m_tool)
		{
			static_cast<NavMeshTesterTool*>(m_tool)->handleKeyStrokes(arg);
		}
	}
...
void OgreTemplate::DebugPrintCurrentState( void )
{
	static int count = 0;
	if( count++ >= 5 )
	{
		dbCompositionList list;
		g_database.ComposeList( list, 0 );
		dbCompositionList::iterator i;
		for( i=list.begin(); i!=list.end(); ++i )
		{
			GameObject * cur = *i;
			StateMachine * mch = cur->GetStateMachine();
			if( mch )
			{
				char name[256];
				strcpy( name, cur->GetName() );
				//printf( "%s:   State: %s   Substate: %s\n", name, 
				//	mch->GetCurrentStateNameString(), mch->GetCurrentSubstateNameString() );
				rcGetLog()->log(RC_LOG_PROGRESS,  "%s:   State: %s   Substate: %s\n", name, 
					mch->GetCurrentStateNameString(), mch->GetCurrentSubstateNameString() );
			}
		}
		count = 0;
